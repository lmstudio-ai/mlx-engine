...
class PromptCacheSession:
    """
    Minimal sequential prompt-cache manager.

    It owns:
    - the live mutable cache used for the current request
    - an LRU history of reusable cache snapshots
    - an optional checkpoint snapshot captured during main-model prefill
      when no draft model is active
    """
    ...
    def _flush_live_cache(self) -> None:
        ...
        cached_tokens = self._live_tokens[:cache_length]
        self._store_snapshot(cached_tokens, self._live_cache)
    ...
    def _restore_cache(
        ...
        cache, rest = self._history.fetch_nearest_cache(
            self._history_key, prompt_tokens.tolist()
        )
        ...
        # Exact hits need one token outside the cache to seed decode. If we
        # cannot trim the exact-hit cache, retry with one less prompt token so
        # a stored checkpoint can win.
    ...
    def prepare(
        ...
        *,
        ...
        if self.verbose:
            print(f"Cached prefix length: {cached_tokens}", file=sys.stderr)
            print(f"Unprocessed tokens: {len(uncached_tokens)}", file=sys.stderr)
        ...
        return PreparedPrompt(
            ...
            uncached_tokens=uncached_tokens[-required_tail_tokens:],
            cached_tokens=cached_tokens,
            total_prompt_tokens=total_prompt_tokens,
        )
