...
class VlmPromptCacheCoordinator:
    def __init__(
        ...
        enqueue_prepared_save: Callable[[PreparedPromptSnapshot], None],
    ):
        self._spill_cache = spill_cache
        self._enqueue_prepared_save = enqueue_prepared_save
    ...
    def restore(
        ...
        *,
        ...
        cached_state = self._spill_cache.load_chunk_sequence(prefix_match.chunk_keys)
    ...
    def make_boundary_callback(
        ...
        *,
        ...
        def _callback(
            ...
            prefix_input_ids = full_prompt_input_ids[:prefix_len]
            chunk = chunks_by_end.get(prefix_len)
            if not prefix_input_ids or chunk is None:
                return
            ...
            prepared_snapshot = self._spill_cache.prepare_save(
                chunk=chunk,
                prompt_input_ids=prefix_input_ids,
                image_hashes=image_hashes,
                min_reusable_prefix_len=min(
                    min_reusable_prefix_len,
                    prefix_len,
                ),
            ...
            if prepared_snapshot is not None:
                self._enqueue_prepared_save(prepared_snapshot)
